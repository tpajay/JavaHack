...
impt java.io.Serializable;
impt java.util.*;
impt java.util.regex.*;
impt javax.servlet.http.HttpServletRequest;
impt org.apache.commons.validator.Field;
impt org.apache.commons.validator.ValidatorAction;
impt org.apache.commons.validator.ValidatorUtil;
impt org.apache.struts.action.ActionErrors;
impt org.apache.struts.action.ActionError;
...
public class AppFormValidator implements Serializable {
	...
	public static boolean validatedatetime( Object bean, ValidatorAction va, Field field,
		...
		if(!theselection.equalsIgnoreCase("sel1")){
		    ...
		    if (value == null || value.length() < 1){
		      errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datetimeerror"));
		    }
			...
			if (value != null && value.length() > 0){
			    ...
			    Matcher m2 = p2.matcher(value);
			  	...
			  	if (m1.matches()){
			  		...
			  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datetimeformaterror"));
			  	}
			  	...
			  	if(isValid){
					...
					if(Integer.parseInt(datetime_month) > 12){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datemontherror"));
						return false;
					}
					if(Integer.parseInt(datetime_day) > 31){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datedayerror"));
						return false;
					}
					if(Integer.parseInt(datetime_hour) > 24){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datehourerror"));
						return false;
					}
					if(Integer.parseInt(datetime_mins) > 59){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.dateminuteserror"));
						return false;
					}
					...
					for (int i=0; i < thirtydaymonths.length;i++){
						if (thirtydaymonths[i] == datetimemonthcheck){
							if(Integer.parseInt(datetime_day) > 30){
								errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datedaysinvalid"));
								return false;
							}
						}
					}
						...
						if ((datetimeyearcheck % 4 == 0) && (!(datetimeyearcheck % 100 == 0)
							|| (datetimeyearcheck % 400) == 0)){
								...
								if(Integer.parseInt(datetime_day) > 29){
									errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datedaysinvalid_isleapyear"));
									return false;
								}
						}else{
							//should be 28days total
							if(Integer.parseInt(datetime_day) > 28){
								errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datedaysinvalid_notleapyear"));
								return false;
							}
						}
					}
					...
					Integer.parseInt(datetime_day), Integer.parseInt(datetime_hour), Integer.parseInt(datetime_mins));
				  	...
				  	if(datetime_date.before(today)){
				  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.datebeforeerror"));
				  	}
				 }
			 }
		}
	...
	public static boolean validateContactNo( Object bean, ValidatorAction va, Field field,
		...
		if(theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3")){
			if (value == null || value.length() < 1){
				errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.contactnoerror"));
		    }
			...
			if (value != null && value.length() > 0){
			    ...
			    Matcher m2 = p2.matcher(value);
			  	...
			  	if (m1.matches()){
			  		...
			  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.contactnoformaterror"));
			  	}
			}
		}
	    return (errors.isEmpty());
		    ...
		    if(value.equalsIgnoreCase("Y")){
				if(distVer.equalsIgnoreCase("1.0") || isWhat.equalsIgnoreCase("Y")) {
					if(ag == null || ag.length() < 1){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agnameerror"));
					}
		    		if(agdate == null || agdate.length() < 1){
		    			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdateerror"));
		    		}
				}
		    }
		    ...
		    if(agdate != null && agdate.length() > 0){
		    	...
		    	boolean isValid = false;
		  		...
		  		if (m1.matches()){
			  		...
			  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdateformaterror"));
			  	}
		  		...
		  		if(isValid){
					...
					if(Integer.parseInt(datetime_month) > 12){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdatemontherror"));
						return false;
					}
					if(Integer.parseInt(datetime_day) > 31){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdatedayerror"));
						return false;
					}
					...
					for (int i=0; i < thirtydaymonths.length;i++){
						if (thirtydaymonths[i] == datetimemonthcheck){
							if(Integer.parseInt(datetime_day) > 30){
								errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdatedaysinvalid"));
								return false;
							}
						}
					}
					...
					if(datetimemonthcheck == 2){
						...
						if ((datetimeyearcheck % 4 == 0) && (!(datetimeyearcheck % 100 == 0)
							|| (datetimeyearcheck % 400) == 0)){
								//then leapyear, should be 29days total
								if(Integer.parseInt(datetime_day) > 29){
									errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdatedaysinvalid_isleapyear"));
									return false;
								}
						}else{
							//should be 28days total
							if(Integer.parseInt(datetime_day) > 28){
								errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.agdatedaysinvalid_notleapyear"));
								return false;
							}
						}
					}
	...
	public static boolean validateSSNTID( Object bean, ValidatorAction va, Field field,
		...
		if((theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3"))){
			...
			if ((value == null || value.length() < 1) && (ssnno == null || ssnno.length() < 1)){
		    	errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.ssnnoerror"));
		    }
			...
			if (value != null && value.length() > 0){
			    if(ssnnotype.equalsIgnoreCase("SSN")){
					...
					if (!m.matches()){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.ssnerror"));
					}
			    }
			    ...
			    if(ssnnotype.equalsIgnoreCase("TAX")){
					...
					Matcher m = p.matcher(value);
					if (!m.matches()){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.taxiderror"));
					}
				}
			}
	...
	public static boolean validateVgn( Object bean, ValidatorAction va, Field field,
		...
		if(theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3")){
		    ...
		    if (pttoorig.equalsIgnoreCase("N")){
		    	if (value == null || value.length() < 1){
		    		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.vgnerror"));
		    	}
		    }
			...
			if (value != null && value.length() > 0){
			    ...
			    Matcher m1 = p1.matcher(value);
			  	...
			  	if (m1.matches()){
			  		...
			  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.vgnformaterror"));
			  	}
			}
			...
			if(theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3")){
				...
				if((firstname == null || firstname.length() < 1) && (lastname == null || lastname.length() < 1)
					&& (value == null || value.length() < 1)){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.firstlastbusinessnameerror"));
	...
	public static boolean validateStreetNo( Object bean, ValidatorAction va, Field field,
		...
		if(theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3")){
		    //if (value == null || value.length() < 1){
		    //  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.streetnoerror"));
		    //}
		}
	    return (errors.isEmpty());
	}
	...
	public static boolean validateStreetName( Object bean, ValidatorAction va, Field field,
		...
		if(distVer.equalsIgnoreCase("1.0") || isWhat.equalsIgnoreCase("Y")) {
			if(theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3")){
				if (value == null || value.length() < 1){
				  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.streetnameerror"));
				}
			}
		}
	    return (errors.isEmpty());
	...
	public static boolean validateCity( Object bean, ValidatorAction va, Field field,
		...
		if(distVer.equalsIgnoreCase("1.0") || isWhat.equalsIgnoreCase("Y")) {
			if(theselection.equalsIgnoreCase("init") || theselection.equalsIgnoreCase("sel3")){
				if (value == null || value.length() < 1){
				  errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.cityerror"));
				}
			}
		}
	    return (errors.isEmpty());
			    ...
			    if(country.equalsIgnoreCase("USA") || country.equalsIgnoreCase("WHT")){
					if(distVer.equalsIgnoreCase("1.0")) {
						if(state == null || state.length() < 1){
							errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.stateerror"));
						}
					}
					//zipcode
					if(value == null || value.length() < 1){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.zipcodeerror"));
					}
				}
				...
				if(value != null && value.length() > 0){
			    	...
			    	if(country.equalsIgnoreCase("WHT")){
			    		if (m3.matches()){
					  		...
					  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.zipcodeWhatformaterror"));
					  	}			    		
			    	}else{
					  	if (m1.matches()){
					  		return true;
					  		...
					  		errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.zipcodeformaterror"));
					  	}
					}
				}
			}else{
					...
					if((String)request.getParameter("country")==null&& (state == null || state.length() < 1)){
						errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.Whatstateerror"));
					}
	...
	public static boolean validateRemarks( Object bean, ValidatorAction va, Field field,
		...
		if(theselection2.length() < 1){
			if(theselection.equalsIgnoreCase("sel3")){
				if (value == null || value.length() < 1){
					errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.remarkserror"));
				}
			}
		}else{
	...
	public static boolean validateComments( Object bean, ValidatorAction va, Field field,
	    ...
	    if (value == null || value.length() < 1){
	      errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("jsp.pt_request.error.commenterror"));
	    }
	    return (errors.isEmpty());
	}
